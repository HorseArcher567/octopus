...
func TestServiceCreateValidation(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), "", "a@b.com"); !errors.Is(err, ErrInvalidArgument) {
		t.Fatalf("expected ErrInvalidArgument for empty username, got: %v", err)
	}
	if _, err := svc.Create(context.Background(), "alice", ""); !errors.Is(err, ErrInvalidArgument) {
